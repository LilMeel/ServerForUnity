...
namespace ServerForGame
{
     public class ServerObject
    {
        ...
        private void Listen()
        {
            try
            {
                var acceptIP = new IPEndPoint(IPAddress.Parse("127.0.0.1"), acceptPort); // Создаем конечную точку на которую будут приходить сообщения
                ...
                while (true)
                {
                    ...
                    do
                    {
                        ...
                        bytes = listeningSocket.ReceiveFrom(data, ref senderIP); // Получим наши 256 байт или меньше из сообщения, их количество
